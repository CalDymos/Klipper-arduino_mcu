...
class KlipperComm {
private:
  Map<uint8_t, void (*)(const char*)> commandMap;  // Map for command callbacks
...
public:
  ...
  bool isConnected = false;  // Becomes true if CMD_IDENTIFY was received and false on CMD_RESTART / CMD_SHUTDOWN
  ...
  void registerCommand(const char* command, void (*func)(const char*)) {
    uint8_t key = (uint8_t)command[0];
    //Serial.print("Registering command: ");
    //Serial.println(key);
    uint8_t index = commandMap.indexOf(key);
    if (index < commandMap.getSize()) {
      commandMap[index](key, func);
    } else {
      for (uint8_t i = 0; i < commandMap.getSize(); i++) {
        if (commandMap[i].getHash() == 0) {  // Empty slot
          commandMap[i](key, func);
          return;
        }
      }
  ...
  char* getParamValue(const char* params, const char* paramKey, bool userParams = false) {
    static char value[32];  // buffer to hold extracted value
    value[0] = '\0';        // ensure buffer is cleared

    char delim = userParams ? ' ' : ';';    // Delimiter based on message type
    char valSep = userParams ? '=' : '\0';  // Separator for key-value pairs (text mode)

    const char* start = strstr(params, paramKey);
    if (!start) return nullptr;  // Parameter not found

    start += strlen(paramKey);                       // Move past paramKey
    if (valSep && *start != valSep) return nullptr;  // Ensure correct key-value format
    if (valSep) start++;                             // Move past '=' if present

    const char* end = strchr(start, delim);  // Find delimiter position
    if (!end) end = start + strlen(start);   // Use full length if no delimiter found

    size_t len = end - start;
    if (len >= sizeof(value)) len = sizeof(value) - 1;  // prevent buffer overflow
    ...
    if ((uint8_t)msgBuffer[0] == (uint8_t)CMD_IDENTIFY[0]) {
      ...
      if (splitPtr) {
        *splitPtr = '\0';  // Separate command from Message
        ...
        void (*func)(const char*) = commandMap.valueOf(key, nullptr);
        if (func) {
          func(params);
          return;
        }
